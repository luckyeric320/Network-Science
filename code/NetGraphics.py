...
def GenerateCircleGraphImage(graph, imfile, windowSize=600,
    ...
    for index, node in enumerate(all_nodes):
        ...
        tx = (radius * 1.1) * np.cos(theta) + center
        ty = (radius * 1.1) * np.sin(theta) + center
...
def GenerateCircleGraphImageWithWeights(graph, imfile,
    ...
    for index, node in enumerate(all_nodes):
        theta = 2. * np.pi * float(index) / L
        x = radius * np.sin(theta) + center
        y = -radius * np.cos(theta) + center
        nodePosition[node] = (x, y)
        if font is not None:
            labelSize = font.getsize(str(node))
            tx = (radius * 1.1) * np.sin(theta) + \
                center - 0.1 * labelSize[0]
            ty = (-radius * 1.1) * np.cos(theta) + \
    ...
    for node in all_nodes:
        ...
        if (node_weights):
            dotsize = dotscale * np.sqrt(node_weights[node] / node_scale)
        else:
            dotsize = dotscale
    ...
    for cluster in nodelists:
        ...
        # Define function to draw thick line
            ...
            def DrawThickLine(sx1, sy1, sx2, sy2):
                perpLength = np.sqrt((sy2 - sy1)**2 + (sx2 - sx1)**2)
...
def DrawTriangularNetworkSites(graph, nodelists=None, L=0,
    ...
    bigHeighty = int(round(magnification * scale * L * np.sqrt(3.) / 2.))
    ...
    xhat = magnification * scale * np.array([1., 0.])
    yhat = magnification * scale * np.array([1. / 2., np.sqrt(3.) / 2.])
    # Shape of polygon
    ...
    upleft = magnification * scale *\
        np.array([-1. / 2., 1. / (2. * np.sqrt(3.))])
    hexagon = np.array([upright, up, upleft, -upright, -up, -upleft])
    # Periodic boundary condition
    pbc = magnification * scale * L * np.array([1., np.sqrt(3.) / 2.])
    ...
    for cluster in nodelists:
        for node in cluster:
            ...
            if (not np.allclose( vertices % pbc, vertices )) or \
               (vertices.min() < 0.):
                # Plot polygons plus pbc vectors
                xcPBC = xc + np.array([pbc[0], 0.])
                vertices = hexagon + xcPBC
                draw.polygon(tuple(map(tuple, vertices)), fill=color)
                xcPBC = xc + np.array([pbc[0] / 2., pbc[1]])
                vertices = hexagon + xcPBC
                draw.polygon(tuple(map(tuple, vertices)), fill=color)
                xcPBC = xc + np.array([-pbc[0] / 2., pbc[1]])
                vertices = hexagon + xcPBC
    ...
    heighty = int(round(scale * L * np.sqrt(3.) / 2.))
